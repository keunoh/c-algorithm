...
- 퀵 정렬은 일반적으로 사용되고 있는 아주 빠른 정렬 알고리즘입니다. 퀵 정렬이라는 이름은 이 알고르짐의 정렬 속도가 매우 빠른데서 착안한 찰스 앤서니 리처드 호어(C. A. R. Hoare)가 직접 붙인 이름입니다. 그룹을 나누는 기준(요소)을 피벗(pivot)이라고 하는데 퀵 정렬은 각 그룹에 대해 피벗 설정과 그룹 나눔을 반복하며 모든 그룹이 한 개가 되면 정렬을 마칩니다.
